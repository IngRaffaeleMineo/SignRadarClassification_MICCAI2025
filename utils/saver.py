...
class Saver(object):
    ...
    def save_model(self, net: torch.nn.Module, optim, name: str, epoch: int):
        """
        Save model parameters in the checkpoint directory.
        """
        # Get state dict
        state_dict = net.state_dict()
        oprim_state_dict = optim.state_dict()
        # Copy to CPU
        for key, value in state_dict.items():
            state_dict[key] = value.cpu()
        # Save
        torch.save(state_dict, str(self.ckpt_path) + '/' + f'{name}_{epoch:05d}.pth')
        torch.save(oprim_state_dict, str(self.ckpt_path) + '/' + f'{name}_optim_{epoch:05d}.pth')
